...
class ProfileSyncService {
    ...
    /**
     * Sync profiles from GUI to CLI using proper Remcli infrastructure
     * SECURITY NOTE: Direct file access is PROHIBITED - use Remcli RPC infrastructure
     */
    public async syncGuiToCli(profiles: AIBackendProfile[]): Promise<void> {
        ...
        try {
            // Profiles are stored in GUI settings and available through existing Remcli sync system
            ...
            console.log(`[ProfileSync] GUI profiles stored in Remcli settings. CLI access via existing infrastructure.`);
            ...
            this.emitEvent({
                ...
                timestamp: Date.now(),
                message: 'Profiles available through Remcli settings system'
            });
        } catch (error) {
    ...
    /**
     * Sync profiles from CLI to GUI using proper Remcli infrastructure
     * SECURITY NOTE: Direct file access is PROHIBITED - use Remcli RPC infrastructure
     */
    public async syncCliToGui(): Promise<AIBackendProfile[]> {
        ...
        try {
            // CLI profiles are accessed through Remcli settings system, not direct file access
            // Return profiles from current GUI settings
            const currentProfiles = storage.getState().settings.profiles || [];

            console.log(`[ProfileSync] Retrieved ${currentProfiles.length} profiles from Remcli settings`);
            ...
            this.emitEvent({
                ...
                timestamp: Date.now(),
                message: 'Profiles retrieved from Remcli settings system'
            });
    ...
    /**
     * Set active profile using Remcli settings infrastructure
     * SECURITY NOTE: Direct file access is PROHIBITED - use Remcli settings system
     */
    public async setActiveProfile(profileId: string): Promise<void> {
        try {
            // Store in GUI settings using Remcli's settings system
            sync.applySettings({ lastUsedProfile: profileId });

            console.log(`[ProfileSync] Set active profile ${profileId} in Remcli settings`);

            // Note: CLI daemon accesses active profile through Remcli settings system
    ...
    /**
     * Get active profile using Remcli settings infrastructure
     * SECURITY NOTE: Direct file access is PROHIBITED - use Remcli settings system
     */
    public async getActiveProfile(): Promise<AIBackendProfile | null> {
        try {
            // Get active profile from Remcli settings system
            ...
            if (activeProfile) {
                console.log(`[ProfileSync] Retrieved active profile ${activeProfile.name} from Remcli settings`);
