...
export class Encryption {
    ...
    static async create(masterSecret: Uint8Array) {
        ...
        const contentDataKey = await deriveKey(masterSecret, 'Remcli Encrypt', ['content']);

        // Derive content data key keypair
        const contentKeyPair = sodium.crypto_box_seed_keypair(contentDataKey);

        // Derive anonymous ID
        const anonID = encodeHex((await deriveKey(masterSecret, 'Remcli', ['analytics', 'id']))).slice(0, 16).toLowerCase();
