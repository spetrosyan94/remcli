...
/**
 * Russian translations for the Remcli app
...
export const ru: TranslationStructure = {
    ...
    settings: {
        ...
        about: 'О программе',
        aboutFooter: 'Remcli — мобильное приложение для работы с Codex и Claude Code. Использует сквозное шифрование, все данные аккаунта хранятся только на вашем устройстве. Не связано с Anthropic.',
        whatsNew: 'Что нового',
    ...
    newSession: {
        ...
        noMachinesFound: 'Машины не найдены. Сначала запустите сессию Remcli на вашем компьютере.',
        ...
        resumeSessionEmpty: 'Нет предыдущих сессий',
        resumeSessionEmptyHint: 'Сначала создайте сессию, потом вы сможете её возобновить',
        resumeSessionUntitled: 'Без названия',
        resumeSessionMessages: ({ count }: { count: number }) => count === 1 ? '1 сообщение' : `${count} сообщений`,
        resumeSessionFailed: 'Не удалось возобновить сессию',
    },
    ...
    sessionInfo: {
        ...
        remcliSessionIdCopied: 'ID сессии Remcli скопирован в буфер обмена',
        failedToCopySessionId: 'Не удалось скопировать ID сессии Remcli',
        remcliSessionId: 'ID сессии Remcli',
        ...
        remcliHome: 'Домашний каталог Remcli',
        ...
        updateCliInstructions: 'Пожалуйста, выполните npm install -g remcli@latest',
    ...
    agentInput: {
        ...
        model: {
            ...
            default: 'По умолчанию',
            defaultDesc: 'Модель агента по умолчанию',
        ...
        cursorPermissionMode: {
            ...
            readOnly: 'Вопрос',
            yolo: 'Форс',
            badgePlan: 'Режим плана',
            badgeReadOnly: 'Режим вопросов',
            badgeYolo: 'Режим форс',
        },
        context: {
    ...
    terminal: {
        ...
        terminalRequestDescription: 'Терминал запрашивает подключение к вашему аккаунту Remcli. Это позволит терминалу безопасно отправлять и получать сообщения.',
    ...
    machine: {
        ...
        offlineHelp: '• Убедитесь, что компьютер online\n• Выполните `remcli daemon status` для диагностики\n• Используете последнюю версию CLI? Обновите командой `npm install -g remcli@latest`',
    ...
    microphone: {
        accessRequired: 'Требуется доступ к микрофону',
        grantPermission: 'Remcli необходим доступ к вашему микрофону для голосового чата. Пожалуйста, предоставьте разрешение при появлении запроса.',
        enableInSettings: 'Remcli необходим доступ к вашему микрофону для голосового чата. Пожалуйста, включите доступ к микрофону в настройках устройства.',
        httpsRequired: 'Для работы микрофона требуется безопасное соединение (HTTPS). Запустите демон с флагом --tunnel для включения голосового ввода: remcli daemon start --tunnel',
        webInstructionsSafari: 'Когда браузер запросит доступ к микрофону, нажмите «Разрешить». Если вы отклонили доступ, перейдите в Настройки \u2192 Safari \u2192 Микрофон и разрешите для этого сайта.',
        webInstructionsChrome: 'Когда браузер запросит доступ к микрофону, нажмите «Разрешить». Если вы отклонили доступ, нажмите на значок настроек сайта (слева от адресной строки) и включите микрофон.',
        webInstructionsGeneric: 'Когда браузер запросит доступ к микрофону, нажмите «Разрешить». Если вы отклонили доступ, проверьте настройки браузера и включите микрофон для этого сайта.',
