...
import { useRemcliAction } from '@/hooks/useRemcliAction';
import { RemcliError } from '@/utils/errors';
...
function SessionInfoContent({ session }: { session: Session }) {
    ...
    const handleCopySessionId = useCallback(async () => {
        ...
        try {
            ...
            Modal.alert(t('common.success'), t('sessionInfo.remcliSessionIdCopied'));
        } catch (error) {
    ...
    // Use RemcliAction for archiving - it handles errors automatically
    const [archivingSession, performArchive] = useRemcliAction(async () => {
        const result = await sessionKill(session.id);
        if (!result.success) {
            throw new RemcliError(result.message || t('sessionInfo.failedToArchiveSession'), false);
        }
    ...
    // Use RemcliAction for deletion - it handles errors automatically
    const [deletingSession, performDelete] = useRemcliAction(async () => {
        const result = await sessionDelete(session.id);
        if (!result.success) {
            throw new RemcliError(result.message || t('sessionInfo.failedToDeleteSession'), false);
        }
    ...
    const handleCopyUpdateCommand = useCallback(async () => {
        const updateCommand = 'npm install -g remcli@latest';
        try {
    ...
    return (
        <>
            <ItemList>
                ...
                <ItemGroup>
                    <Item
                        title={t('sessionInfo.remcliSessionId')}
                ...
                {session.metadata && (
                    <ItemGroup title={t('sessionInfo.metadata')}>
                        ...
                        {session.metadata.remcliHomeDir && (
                            <Item
                                title={t('sessionInfo.remcliHome')}
                                subtitle={formatPathRelativeToHome(session.metadata.remcliHomeDir, session.metadata.homeDir)}
