...
- `npm run prebuild` - Generate native iOS and Android directories
- `npm run typecheck` - Run TypeScript type checking after all changes

### macOS Desktop (Tauri)
- `npm run tauri:dev` - Run macOS desktop app with hot reload
- `npm run tauri:build:dev` - Build development variant
- `npm run tauri:build:preview` - Build preview variant
- `npm run tauri:build:production` - Build production variant
...
### Production
- `npm run ota` - Deploy over-the-air updates via EAS Update to production branch
...
1. **P2P Authentication Flow**: QR code scanned from terminal contains `{mode:'p2p', host, port, key}`. App derives Bearer token from shared secret via HMAC-SHA256 and connects directly to the daemon's P2P server. Legacy cloud QR (`remcli://terminal?...`) is still supported as fallback.
2. **Data Synchronization**: WebSocket-based real-time sync with automatic reconnection. Connects directly to CLI daemon's P2P server (LAN or ngrok tunnel) — no cloud server.
...
7. **P2P Connect** (`sources/sync/p2pConnect.ts`): Parses P2P QR payloads, derives HMAC-SHA256 bearer token, configures server URL for direct daemon connection

### Development Guidelines

- Use **4 spaces** for indentation
- Use **npm run** instead of npm for package management
...
- Always run `npm run typecheck` after all changes to ensure type safety
...
- Always try to use "useRemcliAction" from @sources/hooks/useRemcliAction.ts if you need to run some async operation, do not handle errors, etc - it is handled automatically.
