    ...
    if (state === "transcribing")
        return (
            <div className="flex h-[52px] items-center gap-3 rounded-xl border border-border bg-card px-3.5">
                ...
                <button className="ml-auto font-mono text-[11px] text-muted-foreground/60">{t("voice.cancel")}</button>
            </div>
        );
    if (state === "error")
        return (
            <div className="flex h-[52px] items-center gap-3 rounded-xl border border-status-error/40 bg-status-error/[0.06] px-3.5">
                ...
                <button className="ml-auto h-7 rounded-[7px] bg-secondary px-3 text-xs font-medium">{t("voice.retry")}</button>
            </div>
        );
    return (
