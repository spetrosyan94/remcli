...
describe('REMCLI_CLAUDE_PATH env var', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.REMCLI_CLAUDE_PATH;
  });

  it('should use REMCLI_CLAUDE_PATH when set', () => {
    process.env.REMCLI_CLAUDE_PATH = testClaudePath;
    const result = findGlobalClaudeCliPath();
    expect(result?.source).toBe('REMCLI_CLAUDE_PATH');
  ...
  it('should fall back to auto-discovery when env var not set', () => {
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('REMCLI_CLAUDE_PATH');
  });

  it('should ignore env var if path does not exist', () => {
    process.env.REMCLI_CLAUDE_PATH = '/nonexistent/path/claude';
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('REMCLI_CLAUDE_PATH');
  });
});