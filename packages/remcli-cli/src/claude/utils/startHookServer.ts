 ...
 *     │                                         
 *     ├─► generateHookSettingsFile(port) ──► ~/.remcli/tmp/hooks/session-hook-<pid>.json
 ...
 * 
 * ### Triggered By
 * - `remcli` (fresh start) - new session created
 * - `remcli --continue` - continues last session (may fork)
 * - `remcli --resume` - interactive picker, then resume
 * - `remcli --resume <id>` - resume specific session
 * - `/compact` command - compacts and forks session
 * - Double-escape fork - user forks conversation in CLI
 * 
 * ### Why Not Use File Watching?
 * File watching has race conditions when multiple Remcli processes run.
 * With hooks, Claude directly tells THIS specific process about its session,
 * ensuring 1:1 mapping between Remcli process and Claude session.
 */
