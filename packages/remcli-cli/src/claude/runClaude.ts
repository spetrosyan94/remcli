...
export async function runClaude(credentials: Credentials, options: StartOptions = {}): Promise<void> {
    ...
    if (options.startedBy === 'daemon' && options.startingMode === 'local') {
        throw new Error('Daemon-spawned sessions cannot use local/interactive mode. Use --remcli-starting-mode remote or spawn sessions directly from terminal.');
    }
    ...
    if (!machineId) {
        console.error(`[START] No machine ID found in settings, which is unexpected since authAndSetupMachineIfNeeded should have created it. Please report this issue on https://github.com/spetrosyan94/remcli/issues`);
        process.exit(1);
    }
    ...
    let metadata: Metadata = {
        ...
        remcliToolsDir: resolve(projectPath(), 'tools', 'unpacked'),
    ...
    // In P2P mode, if local server is unreachable it means daemon is not running
    if (!response) {
        throw new Error(
            'Could not create session on local P2P server. ' +
            'Make sure the daemon is running: remcli daemon start'
        );
    }
    ...
    logger.debug(`[START] Remcli MCP server started at ${remcliServer.url}`);
    ...
    const exitCode = await loop({
        ...
        messageQueue,
        api,
        allowedTools: remcliServer.toolNames.map(toolName => `mcp__remcli__${toolName}`),
    ...
    // Stop Remcli MCP server
    remcliServer.stop();
    logger.debug('Stopped Remcli MCP server');
