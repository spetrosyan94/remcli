/**
 * Remcli MCP STDIO Bridge
 *
 * Minimal STDIO MCP server exposing a single tool `change_title`.
 * On invocation it forwards the tool call to an existing Remcli HTTP MCP server
 * using the StreamableHTTPClientTransport.
 *
 * Configure the target HTTP MCP URL via env var `REMCLI_HTTP_MCP_URL` or
...
async function main() {
  ...
  const baseUrl = urlFromArgs || process.env.REMCLI_HTTP_MCP_URL || '';

  if (!baseUrl) {
    // Write to stderr; never stdout.
    process.stderr.write(
      '[remcli-mcp] Missing target URL. Set REMCLI_HTTP_MCP_URL or pass --url <http://127.0.0.1:PORT>\n'
    );
    process.exit(2);
  }
  ...
  async function ensureHttpClient(): Promise<Client> {
    ...
    const client = new Client(
      { name: 'remcli-stdio-bridge', version: '1.0.0' },
      { capabilities: {} }
    );
...
main().catch((err) => {
  try {
    process.stderr.write(`[remcli-mcp] Fatal: ${err instanceof Error ? err.message : String(err)}\n`);
  } finally {
    process.exit(1);
  }
});
