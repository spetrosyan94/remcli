...
export interface OfflineReconnectionConfig<TSession> {
    /** Server URL to health-check against (e.g., 'https://api.remcli-servers.com') */
    serverUrl: string;
    ...
    /**
     * Optional: retry delay calculator.
     * Production uses exponential backoff with jitter; tests can inject a
     * deterministic delay without weakening retry semantics.
     */
...
export function startOfflineReconnection<TSession>(
    ...
    const retryDelayMs = config.retryDelayMs ?? ((failureCount: number) =>
        exponentialBackoffDelay(failureCount, 5000, 60000, 10)
    );
    ...
    const attemptReconnect = async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(e) && e.response?.status === 401) {
                ...
                config.onNotify('❌ Authentication failed. Please re-authenticate with `remcli auth`.');
...
export const ERROR_DESCRIPTIONS: Record<string, string> = {
    ...
    // HTTP errors
    '401': 'authentication failed - run `remcli auth`',
...
class OfflineState {
    ...
    private print(): void {
        const summary = [...this.failures.values()]
            ...
            .join('; ');
        console.log(`⚠️  Remcli server unreachable, offline mode with auto-reconnect enabled - error details: ${summary}`);
