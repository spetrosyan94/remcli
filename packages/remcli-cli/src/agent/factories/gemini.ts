...
export interface GeminiBackendOptions extends AgentFactoryOptions {
  ...
  /** OAuth token from Remcli cloud (via 'remcli connect gemini') - highest priority */
  cloudToken?: string;
...
export function createGeminiBackend(options: GeminiBackendOptions): GeminiBackendResult {
  ...
  // 1. Remcli cloud OAuth token (via 'remcli connect gemini') - highest priority
  ...
  let apiKey = options.cloudToken       // 1. Remcli cloud token (passed from runGemini)
  ...
  if (!apiKey) {
    logger.warn(`[Gemini] No API key found. Run 'remcli connect gemini' to authenticate via Google OAuth, or set ${GEMINI_API_KEY_ENV} environment variable.`);
  }
  ...
  const backendOptions: AcpBackendOptions = {
    ...
    hasChangeTitleInstruction: (prompt: string) => {
      ...
      return lower.includes('change_title') ||
             ...
             lower.includes('mcp__remcli__change_title');
    },
  };
