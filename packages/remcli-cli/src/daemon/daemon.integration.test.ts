...
async function isServerHealthy(): Promise<boolean> {
  try {
    ...
    const testCredentials = existsSync(join(configuration.remcliHomeDir, 'access.key'));
    if (!testCredentials) {
      console.log('[TEST] No test credentials found in', configuration.remcliHomeDir);
      console.log('[TEST] Run "remcli auth login" with REMCLI_HOME_DIR=~/.remcli-dev-test first');
      return false;
    }
...
describe.skipIf(!await isServerHealthy())('Daemon Integration Tests', { timeout: 20_000 }, () => {
  ...
  beforeEach(async () => {
    ...
    void spawnRemcliCLI(['daemon', 'start'], {
      stdio: 'ignore'
    });
  ...
  it('should track session-started webhook from terminal session', async () => {
    ...
    const mockMetadata: Metadata = {
      ...
      host: 'test-host',
      homeDir: '/test/home',
      remcliHomeDir: '/test/remcli-home',
      remcliLibDir: '/test/remcli-lib',
      remcliToolsDir: '/test/remcli-tools',
      hostPid: 99999,
      startedBy: 'terminal',
    ...
    expect(tracked.startedBy).toBe('remcli directly - likely by user from terminal');
    expect(tracked.remcliSessionId).toBe('test-session-123');
  ...
  it('should spawn & stop a session via HTTP (not testing RPC route, but similar enough)', async () => {
    ...
    const spawnedSession = sessions.find(
      (s: any) => s.remcliSessionId === response.sessionId
    );
  ...
  it('should track both daemon-spawned and terminal sessions', async () => {
    // Spawn a real remcli process that looks like it was started from terminal
    const terminalRemcliProcess = spawnRemcliCLI([
    ...
    if (!terminalRemcliProcess || !terminalRemcliProcess.pid) {
      throw new Error('Failed to spawn terminal remcli process');
    }
    ...
    const terminalSession = sessions.find(
      (s: any) => s.pid === terminalRemcliProcess.pid
    );
    const daemonSession = sessions.find(
      (s: any) => s.remcliSessionId === spawnResponse.sessionId
    );
    ...
    try {
      terminalRemcliProcess.kill('SIGTERM');
    } catch (e) {
  ...
  it('should update session metadata when webhook is called', async () => {
    ...
    const sessions = await listDaemonSessions();
    const session = sessions.find((s: any) => s.remcliSessionId === spawnResponse.sessionId);
  ...
  it('should not allow starting a second daemon', async () => {
    ...
    const secondChild = spawn('npx', ['tsx', 'src/index.ts', 'daemon', 'start-sync'], {
      cwd: process.cwd(),
      env: process.env,
  ...
  it('should handle concurrent session operations', async () => {
    ...
    const daemonSessions = sessions.filter(
      (s: any) => s.startedBy === 'daemon' && spawnedSessionIds.includes(s.remcliSessionId)
    );
   ...
   * 3. Test runs `npm run build` to recompile with new version
   ...
   * 6. Daemon spawns new daemon via spawnRemcliCLI(['daemon', 'start'])
   ...
   * 
   * This simulates what happens during `npm upgrade remcli`:
   * - Running daemon has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/remcli/ with NEW version files
   ...
   * - Heartbeat must be long enough (30s) for npm run build to complete before daemon tries to spawn
   * - If heartbeat fires during rebuild, spawn fails (dist/index.mjs missing) and test fails
   * - pkgroll doesn't reliably update compiled version, must use full npm run build
  ...
  it('[takes 1 minute to run] should detect version mismatch and kill old daemon', { timeout: 100_000 }, async () => {
    ...
    try {
      ...
      // We are not using npm run build here because it cleans out dist/
      // and we want to avoid that, 
      // otherwise daemon will spawn a non existing remcli js script.
      ...
      await new Promise(resolve => setTimeout(resolve, parseInt(process.env.REMCLI_DAEMON_HEARTBEAT_INTERVAL || '30000') + 10_000));
  ...
  // TODO: Test npm uninstall scenario - daemon should gracefully handle when remcli is uninstalled
