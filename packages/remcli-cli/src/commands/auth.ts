...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('remcli auth')} - Authentication management

${chalk.bold('Usage:')}
  remcli auth login [--force]    Authenticate with Remcli
  remcli auth logout             Remove authentication and machine data
  remcli auth status             Show authentication status
  remcli auth help               Show this help message
...
async function handleAuthLogin(args: string[]): Promise<void> {
  ...
  if (!forceAuth) {
    ...
    if (existingCreds && settings?.machineId) {
      ...
      console.log(chalk.gray(`  Use 'remcli auth login --force' to re-authenticate`));
      return;
...
async function handleAuthLogout(): Promise<void> {
  ...
  const remcliDir = configuration.remcliHomeDir;
  ...
  console.log(chalk.blue('This will log you out of Remcli'));
  console.log(chalk.yellow('⚠️  You will need to re-authenticate to use Remcli again'));
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      ...
      if (existsSync(remcliDir)) {
        rmSync(remcliDir, { recursive: true, force: true });
      }

      console.log(chalk.green('✓ Successfully logged out'));
      console.log(chalk.gray('  Run "remcli auth login" to authenticate again'));
    } catch (error) {
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "remcli auth login" to authenticate'));
    return;
  }
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "remcli auth login --force" to fix this'));
  }

  // Data location
  console.log(chalk.gray(`\n  Data directory: ${configuration.remcliHomeDir}`));
