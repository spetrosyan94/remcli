...
Опциональный LLM-ассистент внутри демона (выключен по умолчанию: `conciergeEnabled` в `~/.remcli/setup.json`). Подключается к LM Studio (или любому OpenAI-совместимому API, по умолчанию `http://127.0.0.1:1234/v1`) и через function calling умеет:
