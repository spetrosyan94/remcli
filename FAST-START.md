...
QR сканируется один раз: секрет и порт сохраняются в `~/.remcli/p2p-pairing.json`, поэтому подключение переживает рестарты демона. Сменить ключ (все устройства должны пересканировать QR): `remcli daemon rekey`.
