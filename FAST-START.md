...
QR сканируется один раз: pairing-данные и порт сохраняются в `~/.remcli/p2p-pairing.json`, поэтому подключение переживает рестарты демона. Сменить ключ удалённого доступа (все устройства должны пересканировать QR): `remcli daemon rekey`.
