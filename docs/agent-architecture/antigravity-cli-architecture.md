...
Штатный Remote Control работает с Google UI и не входит в пользовательский flow
Remcli. Local terminal frontend и phone/web работают поверх одного daemon-owned
`agy stream-json` по
[общему single-writer контракту](shared-live-chat-architecture.md). Terminal UI
строится на `@earendil-works/pi-tui` только как rendering/input library; Pi
agent runtime и Pi provider не подключаются. Штатный TUI не зеркалируется через
ANSI/PTY, а второй writer для той же conversation не запускается.
